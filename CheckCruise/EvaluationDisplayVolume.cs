...
{
    class EvaluationDisplayVolume : CheckCruiseReports
    {
        #region
            private EvaluationReportRTF eRTF = new EvaluationReportRTF();
            private int includeFlag = 0;
            private int compareResult = 0;
        #endregion


        public int DisplayVolume(StreamWriter strWriteOut,  TolerancesList currTolerance, string cruiserInitials)
        {
            ...
            {
                groupedTrees = db.getResultsTable("R_TreeSpecies","");
                byWhich = 1;
            }
            else if (currTolerance.T_ByProduct == 1)
            {
                groupedTrees = db.getResultsTable("R_TreeProduct","");
                byWhich = 2;
            }   //  endif

            //  complete and print header for volume portion
            int tableToUse = CompleteAndPrintHeader(currTolerance,strWriteOut);
            ...
            foreach (ResultsList gt in groupedTrees)
            {
                ...
                if (currentSpeciesVolume.Count > 0)
                {
                    ...
                    //  print volume lines
                    printLines(strWriteOut, eRTF.VolumeLine, 41);
        ...
        private int CompleteAndPrintHeader(TolerancesList currTolerance, StreamWriter strWriteOut)
        {
            int tableType = 0;
            ...
            if (includeFlag == 0)
            {
                if (currTolerance.T_CUFTPGtolerance == 1 && currTolerance.T_BDFTPGtolerance == 1)
                {
                    ...
                    printLines(strWriteOut, eRTF.VolumeHeaderPrimary, 28);
                    tableType = 1;
                }
                else if (currTolerance.T_CUFTSGtolerance == 1 && currTolerance.T_BDFTSGtolerance == 1)
                {
                    ...
                    printLines(strWriteOut, eRTF.VolumeHeaderSecondary, 28);
                    tableType = 2;
                }
                else if (currTolerance.T_CUFTPSGtolerance == 1 && currTolerance.T_BDFTPSGtolerance == 1)
                {
                    ...
                    tableType = 3;
                    printLines(strWriteOut, eRTF.VolumeHeaderBoth, 19);
                }   //  endif
            }
            ...
            printLines(strWriteOut, eRTF.CruiserCheckHeader, 42);
            return tableType;
